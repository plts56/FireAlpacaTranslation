...
QString Trans_Str3D( int idx )
{
  ...
  if (idx == 30) return QObject::tr("Кликнуть для выбора активного объекта"); // クリックでアクティブなオブジェクトを選択
  ...
  if (idx == 34) return QObject::tr("Сцена не может управляться"); // ステージは操作できません
