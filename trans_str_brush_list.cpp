...
QString Trans_StrBrushList( int idx )
{
  ...
  if (man->TranslateRussian())
  {
    ...
    if (idx == 15) return QObject::tr("Лист");
    if (idx == 16) return QObject::tr("Цветок");
    if (idx == 17) return QObject::tr("Частица");
    ...
    if (idx == 31) return QObject::tr("Экспорт файла кисти");
    if (idx == 32) return QObject::tr("Импорт файлов кистей");

    if (idx == 40) return QObject::tr(" (Жесткий)");
    if (idx == 41) return QObject::tr(" (Мягкий)");
    if (idx == 42) return QObject::tr(" (Canvas)");
    if (idx == 43) return QObject::tr(" (Sketchbook)");
    if (idx == 44) return QObject::tr(" (Bleeding)");

    if (idx == 100) return QObject::tr("Маркер");
